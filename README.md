...
MySQL 保存天气、火车票、机票和演唱会数据。当前 Agent 流程使用天气表和火车票表。
...
| `scripts/import_csv_data.py` | 将天气、火车票、机票和演唱会 CSV 数据批量写入 MySQL，并通过唯一键执行更新或插入。 |
...
| `flight_tickets` | 航班与余票数据 | 数据已预留 |
| `concert_tickets` | 演唱会票务数据 | 数据已预留 |
...
`data/` 目录中的天气、火车票、机票和演唱会 CSV 可以通过以下命令批量导入：
...
- 自动识别 `weather_data_*.csv`、`train_tickets_*.csv`、`flight_tickets_*.csv` 和 `concert_tickets_*.csv`
- 使用 UTF-8 读取中文数据
- 自动创建缺失的数据表
- 忽略 CSV 中的自增 `id`
